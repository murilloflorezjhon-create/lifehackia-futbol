...
# ─── CONFIGURACIÓN ────────────────────────────────────────────
API_KEY = "69a4800800db4cab9548fe84e5f35953"  # Regístrate gratis en football-data.org
...
def obtener_partidos_proximos(liga_code: str) -> list:
    """
    Descarga los próximos partidos programados de una liga.
    """
    url = f"{BASE_URL}/competitions/{liga_code}/matches"
    params = {
        "status": "SCHEDULED"
    ...
    print(f"📡 Descargando próximos partidos de {LIGAS.get(liga_code, liga_code)}...")
    ...
    try:
        ...
        if response.status_code == 200:
            data = response.json()
            partidos = data.get("matches", [])
            print(f"   ✅ {len(partidos)} partidos programados encontrados")
            return partidos
        elif response.status_code == 429:
            print("   ⚠️  Límite de solicitudes alcanzado. Esperando 60 segundos...")
            time.sleep(60)
            return obtener_partidos_proximos(liga_code)
        else:
            print(f"   ❌ Error {response.status_code}: {response.text}")
            return []
...
def procesar_partidos_proximos(partidos_programados: list) -> pd.DataFrame:
    """
    Convierte los partidos programados en un DataFrame que se puede consumir desde el frontend.
    """
    registros = []
    ...
    for p in partidos_programados:
        try:
            registro = {
                ...
                "hora":            p["utcDate"][11:16] if p.get("utcDate") else "",
            }
            registros.append(registro)
        except (KeyError, TypeError):
            continue
    ...
    df = pd.DataFrame(registros)
    print(f"   📊 DataFrame de próximos partidos creado: {len(df)} filas")
    return df
...
if __name__ == "__main__":
    ...
    if API_KEY != "TU_API_KEY_AQUI":
        ...
        if todos_partidos:
            ...
            guardar_csv(df_total, "partidos_raw")
            
            df_features = calcular_features(df_total)
            guardar_csv(df_features, "partidos_features")

        todos_programados = []
        for codigo_liga in ["PD", "PL", "BL1"]:
            partidos_programados = obtener_partidos_proximos(codigo_liga)
            if partidos_programados:
                df_programados = procesar_partidos_proximos(partidos_programados)
                if not df_programados.empty:
                    todos_programados.append(df_programados)
            time.sleep(6)

        if todos_programados:
            df_upcoming = pd.concat(todos_programados, ignore_index=True)
            guardar_csv(df_upcoming, "partidos_upcoming")
        else:
            print("   ⚠️ No se encontraron próximos partidos programados.")
    ...
    # Opción 2: Datos de demo (sin API Key)
        ...
        guardar_csv(df_demo, "partidos_raw")
