...
const MATCHES = [
  { id:1, league:"LaLiga",          flag:"🇪🇸", home:"Real Madrid",    homeLogo:"🤍", away:"Barcelona",    awayLogo:"❤️", time:"21:00" },
  { id:2, league:"Premier League",  flag:"🏴󠁧󠁢󠁥󠁮󠁧󠁿", home:"Arsenal",        homeLogo:"🔴", away:"Chelsea",      awayLogo:"🔵", time:"17:30" },
  { id:3, league:"Liga BetPlay",    flag:"🇨🇴", home:"América de Cali", homeLogo:"🔴", away:"Millonarios",  awayLogo:"💙", time:"18:10" },
  { id:4, league:"Champions",       flag:"⭐",  home:"Bayern Munich",   homeLogo:"🔴", away:"PSG",          awayLogo:"🔵", time:"21:00" },
  { id:5, league:"Serie A",         flag:"🇮🇹", home:"Inter Milan",     homeLogo:"⚫", away:"Napoli",       awayLogo:"💙", time:"20:45" },
  { id:6, league:"LaLiga",          flag:"🇪🇸", home:"Atlético Madrid", homeLogo:"🔴", away:"Sevilla",      awayLogo:"⚪", time:"19:00" },
];
...
export default function App() {
  ...
  const [showAnalysis, setShowAnalysis] = useState(false); // mobile: toggle panel
  ...
  const swipe = (dir) => {
    ...
    setTimeout(()=>{ setSwipeAnim(null); setSwipeIdx(i=>(i+1)%MATCHES.length); },350);
  };
  ...
  const winnerName = pred&&selected ? (pred.prediccion==="LOCAL"?selected.home:pred.prediccion==="VISITANTE"?selected.away:"EMPATE") : "";
  const winnerColor = pred ? (pred.prediccion==="LOCAL"?"#22c55e":pred.prediccion==="VISITANTE"?"#ef4444":G) : G;

  // ─── SHARED COMPONENTS ────────────────────────────────────────

  const MatchItem = ({m, compact=false}) => (
    <div onClick={()=>pickMatch(m)} style={{
      ...
      <div style={{fontSize:9,color:G,letterSpacing:.8,textTransform:"uppercase",marginBottom:5}}>
        {m.flag} {m.league}
      </div>
      <div style={{display:"flex",alignItems:"center",justifyContent:"space-between",marginBottom:4}}>
        <div style={{display:"flex",alignItems:"center",gap:5}}>
          <span style={{fontSize:compact?13:15}}>{m.homeLogo}</span>
          <span style={{fontSize:compact?11:12,fontWeight:600}}>{m.home}</span>
        </div>
        <span style={{fontSize:9,fontWeight:700,color:"rgba(201,168,76,0.45)",background:"rgba(201,168,76,0.07)",borderRadius:4,padding:"2px 5px"}}>VS</span>
        <div style={{display:"flex",alignItems:"center",gap:5}}>
          <span style={{fontSize:compact?11:12,fontWeight:600}}>{m.away}</span>
          <span style={{fontSize:compact?13:15}}>{m.awayLogo}</span>
        </div>
      </div>
  ...
  const AnalysisPanel = () => (
    <div style={{padding: isMobile?"16px":"20px"}}>
      ...
      <div style={{display:"grid",gridTemplateColumns:`repeat(${isMobile?2:4},1fr)`,gap:10,marginBottom:18}}>
        {[[MATCHES.length,"Partidos hoy"],["61%","Precisión IA"],["3 🔥","Tu racha"],["#42","Ranking"]].map(([v,l])=>(
          <div key={l} style={{background:"#111",border:`1px solid rgba(201,168,76,0.1)`,borderRadius:12,padding:"12px 10px",textAlign:"center"}}>
            <div style={{fontSize:isMobile?18:20,fontWeight:800,color:G,fontFamily:"Georgia,serif"}}>{v}</div>
      ...
      {/* ANALYSIS CARD */}
      {selected ? (
        <div style={{background:"#111",border:`1px solid rgba(201,168,76,0.15)`,borderRadius:16,padding:isMobile?16:20,marginBottom:14}}>
          ...
          <div style={{display:"flex",alignItems:"flex-start",justifyContent:"space-between",marginBottom:14,paddingBottom:12,borderBottom:"1px solid rgba(201,168,76,0.07)"}}>
            <div>
              ...
              <div style={{fontFamily:"Georgia,serif",fontSize:isMobile?15:18,fontWeight:800,lineHeight:1.3}}>{selected.home} vs {selected.away}</div>
            </div>
            <div style={{background:"rgba(201,168,76,0.09)",border:`1px solid rgba(201,168,76,0.2)`,borderRadius:20,padding:"4px 10px",fontSize:9,color:G,fontWeight:600,whiteSpace:"nowrap",marginLeft:8}}>
              {selected.flag} {selected.league}
            </div>
          </div>

          {/* TEAMS */}
          <div style={{display:"grid",gridTemplateColumns:"1fr auto 1fr",alignItems:"center",gap:12,background:"#161616",borderRadius:12,padding:"14px 12px",marginBottom:16}}>
            <div style={{textAlign:"center"}}>
              <div style={{fontSize:isMobile?32:40,marginBottom:5}}>{selected.homeLogo}</div>
              <div style={{fontSize:isMobile?11:13,fontWeight:600,lineHeight:1.3}}>{selected.home}</div>
              <div style={{fontSize:9,color:"rgba(255,255,255,0.28)",marginTop:2}}>Local</div>
            </div>
            <div style={{fontFamily:"Georgia,serif",fontSize:isMobile?18:22,fontWeight:900,color:G,background:"rgba(201,168,76,0.09)",border:`1px solid rgba(201,168,76,0.2)`,borderRadius:10,padding:"6px 10px"}}>VS</div>
            <div style={{textAlign:"center"}}>
              <div style={{fontSize:isMobile?32:40,marginBottom:5}}>{selected.awayLogo}</div>
              <div style={{fontSize:isMobile?11:13,fontWeight:600,lineHeight:1.3}}>{selected.away}</div>
              <div style={{fontSize:9,color:"rgba(255,255,255,0.28)",marginTop:2}}>Visitante</div>
            </div>
          </div>
          ...
          {!loading && pred && (
            <>
              <div style={{textAlign:"center",marginBottom:16}}>
                <div style={{fontSize:9,color:"rgba(201,168,76,0.55)",letterSpacing:2,marginBottom:5}}>PREDICCIÓN</div>
              ...
              <div style={{background:"#161616",borderLeft:`3px solid ${G}`,borderRadius:"0 10px 10px 0",padding:"11px 14px",margin:"14px 0",fontSize:12,color:"rgba(255,255,255,0.55)",lineHeight:1.6}}>
                {pred.advertencia} Análisis basado en forma reciente, H2H y estadísticas de los últimos 5 partidos.
              </div>
          ...
          {!loading&&!pred&&(
            <div style={{textAlign:"center",padding:"16px 0",color:"rgba(255,255,255,0.25)",fontSize:13}}>Cargando análisis…</div>
          )}
        </div>
      ) : (
        ...
        {LEADERBOARD.map(p=>(
          <div key={p.rank} style={{display:"flex",alignItems:"center",gap:12,padding:"9px 0",borderBottom:"1px solid rgba(255,255,255,0.035)"}}>
            ...
            <div style={{flex:1,minWidth:0}}>
              <div style={{fontSize:12,fontWeight:600,overflow:"hidden",textOverflow:"ellipsis",whiteSpace:"nowrap"}}>{p.country} {p.name}</div>
              <div style={{fontSize:10,color:"rgba(255,255,255,0.3)"}}>{p.streak} racha</div>
            </div>
  ...
  const SwipeTab = () => (
    <div style={{maxWidth:400,margin:"0 auto",padding:"24px 20px"}}>
      <div style={{textAlign:"center",marginBottom:22}}>
        ...
        <div style={{fontSize:12,color:"rgba(255,255,255,0.35)"}}>Vota antes de ver la predicción</div>
      </div>

      <div style={{
        background:"#111",border:`1px solid rgba(201,168,76,0.18)`,borderRadius:22,
        padding:"24px 20px",textAlign:"center",
        transform:swipeAnim==="left"?"translateX(-110%) rotate(-12deg)":swipeAnim==="right"?"translateX(110%) rotate(12deg)":"none",
        opacity:swipeAnim?0:1,transition:"transform .35s,opacity .3s",
      }}>
        <div style={{fontSize:10,color:G,letterSpacing:1.5,textTransform:"uppercase",marginBottom:16}}>{MATCHES[swipeIdx]?.flag} {MATCHES[swipeIdx]?.league}</div>
      ...
      <div style={{display:"flex",gap:16,justifyContent:"center",marginTop:20}}>
        {[["←","#ef4444","left"],["🤝",G,"empate"],["→","#22c55e","right"]].map(([icon,color,dir])=>(
          <button key={dir} onClick={()=>swipe(dir)} style={{width:60,height:60,borderRadius:"50%",border:`2px solid ${color}`,background:"transparent",color,fontSize:20,cursor:"pointer",transition:"all .18s",display:"flex",alignItems:"center",justifyContent:"center"}}>
            {icon}
          </button>
        ))}
      </div>
        ...
        <div style={{marginTop:24}}>
          ...
          {votes.slice(-4).reverse().map((v,i)=>(
            <div key={i} style={{background:"#111",border:`1px solid rgba(201,168,76,0.08)`,borderRadius:10,padding:"10px 14px",marginBottom:8,display:"flex",justifyContent:"space-between",alignItems:"center"}}>
              ...
              <span style={{fontSize:9,padding:"2px 8px",borderRadius:10,background:"rgba(201,168,76,0.12)",color:G,fontWeight:600}}>
  ...
  return (
    <div style={{minHeight:"100vh",background:"#0a0a0a",color:"#fff",fontFamily:"'DM Sans',system-ui,sans-serif"}}>
      ...
      {/* ── HEADER ── */}
      <header style={{
        background:"#111",borderBottom:`1px solid rgba(201,168,76,0.18)`,
        padding:`0 ${isMobile?"14px":"24px"}`,height:56,
        ...
        <div style={{display:"flex",alignItems:"center",gap:isMobile?8:14}}>
          <div style={{display:"flex",alignItems:"center",gap:5,background:"rgba(34,197,94,0.08)",border:"1px solid rgba(34,197,94,0.25)",borderRadius:20,padding:"4px 10px",fontSize:10,color:"#22c55e"}}>
            ...
            {isMobile?"Online":apiOk===null?"Conectando…":apiOk?"IA Online":"Demo"}
          </div>
      ...
      {/* ── NAV ── */}
      <nav style={{background:"#111",borderBottom:`1px solid rgba(201,168,76,0.1)`,padding:`0 ${isMobile?"8px":"20px"}`,display:"flex",gap:0,overflowX:"auto"}}>
        {[["partidos","🗓️","Partidos hoy"],["swipe","👆","¿Quién gana?"],["ranking","🏆","Ranking"]].map(([id,icon,lbl])=>(
          <button key={id} onClick={()=>setTab(id)} style={{
            padding:isMobile?"10px 12px":"10px 18px",
            fontSize:isMobile?11:12,fontWeight:500,
            color:tab===id?G:"rgba(255,255,255,0.35)",
            ...
            {icon} {!isMobile&&lbl}
            {isMobile&&<div style={{fontSize:9,marginTop:2,color:"inherit"}}>{lbl}</div>}
          </button>
        ))}
      </nav>
          ...
          {!isMobile && (
            <div style={{display:"grid",gridTemplateColumns:isTablet?"260px 1fr":"290px 1fr",minHeight:"calc(100vh - 102px)"}}>
              ...
              <aside style={{background:"#111",borderRight:`1px solid rgba(201,168,76,0.1)`,overflowY:"auto"}}>
                <div style={{padding:"13px 14px 10px",borderBottom:`1px solid rgba(201,168,76,0.07)`,display:"flex",alignItems:"center",justifyContent:"space-between"}}>
                  <span style={{fontSize:13,fontWeight:700,color:"#fff",fontFamily:"Georgia,serif"}}>Partidos</span>
                  <span style={{background:"rgba(201,168,76,0.12)",border:`1px solid rgba(201,168,76,0.25)`,borderRadius:20,padding:"2px 9px",fontSize:10,color:G,fontWeight:600}}>{MATCHES.length} hoy</span>
                </div>
                {MATCHES.map(m=><MatchItem key={m.id} m={m} compact={isTablet}/>)}
              </aside>
              {/* RIGHT ANALYSIS */}
              ...
              {!showAnalysis ? (
                ...
                <div>
                  <div style={{padding:"12px 14px 8px",borderBottom:`1px solid rgba(201,168,76,0.07)`,display:"flex",alignItems:"center",justifyContent:"space-between",background:"#111"}}>
                    ...
                    <span style={{background:"rgba(201,168,76,0.12)",border:`1px solid rgba(201,168,76,0.25)`,borderRadius:20,padding:"2px 9px",fontSize:10,color:G,fontWeight:600}}>{MATCHES.length}</span>
                  </div>
                  {/* STATS MINI */}
                  <div style={{display:"grid",gridTemplateColumns:"repeat(2,1fr)",gap:8,padding:"12px 14px",background:"#0a0a0a"}}>
                    {[["61%","Precisión IA"],["3 🔥","Tu racha"]].map(([v,l])=>(
                      <div key={l} style={{background:"#111",border:`1px solid rgba(201,168,76,0.1)`,borderRadius:10,padding:"10px",textAlign:"center"}}>
                        <div style={{fontSize:18,fontWeight:800,color:G,fontFamily:"Georgia,serif"}}>{v}</div>
                        <div style={{fontSize:9,color:"rgba(255,255,255,0.32)",marginTop:2}}>{l}</div>
                      </div>
                    ))}
                  </div>
      ...
      {/* MOBILE BOTTOM NAV (extra UX) */}
      {isMobile && (
        <div style={{position:"fixed",bottom:0,left:0,right:0,background:"#111",borderTop:`1px solid rgba(201,168,76,0.15)`,display:"flex",zIndex:50,height:56}}>
          {[["partidos","🗓️","Hoy"],["swipe","👆","Votar"],["ranking","🏆","Ranking"]].map(([id,icon,lbl])=>(
            <button key={id} onClick={()=>{setTab(id);if(id!=="partidos")setShowAnalysis(false);}} style={{
              flex:1,display:"flex",flexDirection:"column",alignItems:"center",justifyContent:"center",gap:2,
              background:"none",border:"none",
              color:tab===id?G:"rgba(255,255,255,0.35)",
              cursor:"pointer",transition:"color .2s",fontSize:18,
            }}>
              {icon}
